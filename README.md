...
# 2. Install dependencies AND the batteryplot package (creates the CLI entry point)
pip install -e .

# 3. Verify
batteryplot --help

# 4. Put your CSV files in the input/ folder, then:
batteryplot run
```
...
> **Why `pip install -e .` and not just `pip install -r requirements.txt`?**
> `requirements.txt` only installs the third-party libraries. The `batteryplot`
> command itself comes from the `[project.scripts]` entry point defined in
> `pyproject.toml`, which is only registered when you install the package.
> `pip install -e .` does both steps at once (editable / development install).
